// Aonyx Timer — In-Page Script (DE)
// Credits: basiert auf erprobten Routinen (PornoPommes).

(function(){
  'use strict';

  // --- Reality check: nur im Spielkontext sinnvoll.
  if (!window.TribalWars || typeof TribalWars.post !== 'function' || !window.game_data) {
    alert('Aonyx Timer: Im Spiel ausführen. (Versammlungsplatz/Map, echter Tab)');
    return;
  }

  /* ===== kleine Helfer ===== */
  const LS = 'aonyx_timer_v1_';
  ...
  function parseTime(s){
    ...
    if(!m) return null;
    const d=new Date(); d.setHours(+m[1]||0, +m[2]||0, +m[3]||0, Number((m[4]||'0').padEnd(3,'0')));
    return d;
  }
  ...
  // Units? Frag das Spiel, kein Horoskop.
  function availableUnits(){
    if (Array.isArray(game_data.units) && game_data.units.length) return game_data.units.slice();
    return ['spear','sword','axe','archer','spy','light','marcher','heavy','ram','catapult','knight']; // fallback-welt
  }
  const UNIT_LABEL = {
  ...
  wrap.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;border-bottom:1px solid #eee">
      ...
      <div style="display:flex;gap:8px;align-items:center">
        <button id="ax_toggle" style="padding:4px 8px;border-radius:6px">Ein-/Ausklappen</button>
        <button id="ax_close" style="padding:4px 8px;border-radius:6px;background:#ffe9e9;border:1px solid #c33">X</button>
      </div>
    </div>
    <div id="ax_body" style="display:none;padding:10px">
      ...
      <div id="ax_table_wrap" style="margin-top:10px;max-height:420px;overflow:auto;border-top:1px solid #eee;padding-top:8px"></div>
      <div style="margin-top:10px;display:flex;gap:10px;align-items:center">
        ...
        <small style="color:#666">Ja, der Knopf ist scharf.</small>
      </div>
    </div>
  `;
  ...
  // Countdown (weil Sekunden immer schneller verfliegen als Nerven)
  setInterval(()=>{
  ...
  // Dörfer: erst am_farm, dann overview_villages + map/village.txt
      ...
      TribalWars.post('game.php', { screen:'am_farm', ajax:'1', ajaxaction:'list', village: game_data.village.id }, function(resp){
        ...
        TribalWars.get('game.php', { screen:'overview_villages', mode:'combined', village: game_data.village.id }, function(html){
          try{
            ...
            const links=Array.from(doc.querySelectorAll('a[href*="village="]'));
            const seen=new Set(), v=[];
            links.forEach(a=>{
              const m=(a.getAttribute('href')||'').match(/village=(\d+)/);
              if(m && !seen.has(m[1])){ seen.add(m[1]); v.push({ id:m[1], name:a.textContent.trim(), x:0, y:0 }); }
            });
            // Koords über village.txt
            jQuery.get(location.origin+'/map/village.txt', function(txt){
              ...
              const withCoords=v.map(vv=> idx.has(vv.id)? Object.assign(vv, idx.get(vv.id)) : vv);
              save('vills', withCoords); resolve(withCoords);
  ...
  // Units pro Dorf: am_farm-Varianten, sonst place scrapen
  function loadUnitsForVillage(vid){
    return new Promise((resolve)=>{
      const actions=['get_farm_units','get_units','get_farm_units_v2'];
      let i=0;
      function step(){
        if(i>=actions.length){
          // Fallback: place
          TribalWars.get('game.php', { screen:'place', village: vid }, function(html){
            try{
              ...
              availableUnits().forEach(u=>{
                const s=doc.querySelector(`#units_entry_all_${u}, #units_entry_${u}, span[id*="units_entry"][id*="${u}"]`);
                if(s){
                  const m=s.textContent.replace(/\./g,'').match(/(\d+)/);
                  if(m) out[u]=+m[1];
                }
              });
              resolve(out);
        ...
        TribalWars.post('game.php', { screen:'am_farm', ajax:'1', ajaxaction:action, village: vid }, function(resp){
          try{
            const j=(typeof resp==='string')?JSON.parse(resp):resp;
            const u=j?.units || j?.response?.units || {};
            if(u && Object.keys(u).length){ resolve(u); }
            else step();
          }catch(_){ step(); }
        });
      }
  ...
  // Echte Laufzeit: Kontext auf place für das Dorf -> calculate_time
  function calcDurationMs(vid, unit, tx, ty){
    return new Promise((resolve)=>{
      TribalWars.get('game.php', { village: vid, screen:'place' }, function(_html){
        TribalWars.post('game.php', {
          village: vid, screen:'place', ajax:'1', ajaxaction:'calculate_time',
  ...
  // Losschicken (nur wenn du GO drückst). Keine Ausreden.
  function sendCommand(vid, tx, ty, type, unitsObj){
    const payload = Object.assign({
      village: vid, screen:'place', ajax:'1', ajaxaction:'command',
      x: tx, y: ty, type: type, h: game_data.csrf
    }, Object.fromEntries(Object.entries(unitsObj).map(([k,v])=>[`units[${k}]`, v])));
    TribalWars.post('game.php', payload, function(resp){
      try { UI.SuccessMessage('Aonyx: Befehl gesendet (Konsole).'); } catch(_) {}
      console.log('[Aonyx] send response:', resp);
    });
  }

  /* ===== Tabelle / Events ===== */
  function buildTable(vills){
    const w=$('#ax_table_wrap');
    const units=availableUnits();
    let html='<table class="vis" style="width:100%;border-collapse:collapse"><thead><tr>'
      +'<th style="padding:6px">Dorf / Koords</th>'
      +'<th style="padding:6px">Einheit</th>'
      +'<th style="padding:6px">Verfügbar</th>'
      +'<th style="padding:6px">Laufzeit</th>'
      +'<th style="padding:6px">Senden</th>'
    ...
    for(const v of vills){
      ...
      for(const u of units){
        ...
        const amt   = load('amounts', {})?.[v.id]?.[u] ?? 0;
        html+=`<tr>
          <td style="padding:6px">${UNIT_LABEL[u]||u}</td>
          <td style="padding:6px;color:#666">${u}</td>
          <td style="padding:6px"><span id="ax_av_${v.id}_${u}">${avail}</span></td>
          <td id="ax_time_${v.id}_${u}" style="padding:6px">-</td>
          <td style="padding:6px"><input class="ax_amt" data-vid="${v.id}" data-unit="${u}" value="${amt}" style="width:70px"></td>
          <td style="padding:6px"><button class="ax_calc btn" data-vid="${v.id}" data-unit="${u}">calc</button></td>
        </tr>`;
      }
    }
    html+='</tbody></table>';
    ...
    w.querySelectorAll('.ax_chk').forEach(btn=>{
      btn.addEventListener('click', async ()=>{
        ...
        try{ UI.SuccessMessage('Units…'); }catch(_){}
    ...
    w.querySelectorAll('.ax_calc').forEach(btn=>{
      btn.addEventListener('click', async ()=>{
        ...
        const coord=($('#ax_target').value||'').trim(); if(!coord) return UI.ErrorMessage('Kein Ziel');
        const m=coord.match(/^(\d{1,3})\|(\d{1,3})$/); if(!m) return UI.ErrorMessage('Ziel kaputt');
        const tx=+m[1], ty=+m[2];
        const cell=$(`ax_time_${vid}_${unit}`); if(cell) cell.textContent='…';
        const ms = await calcDurationMs(vid, unit, tx, ty);
        cell.textContent = ms ? `${Math.round(ms/1000)}s` : 'n/a';
        const tgt=parseTime($('#ax_time').value);
        const ok=(tgt&&ms)?(tgt.getTime()-ms>Date.now()):null;
  ...
  /* ===== Buttons ===== */
  $('#ax_load_vills').onclick = async ()=>{
    const v=await loadVillages();
    if(!v||!v.length){ try{UI.ErrorMessage('Keine Dörfer.')}catch(_){}
      return;
    }
    try{ UI.SuccessMessage(`Dörfer: ${v.length}`); }catch(_){}
    buildTable(v);
  };

  $('#ax_refresh').onclick = async ()=>{
    const coord=($('#ax_target').value||'').trim(); if(!coord) return UI.ErrorMessage('Kein Ziel');
    const m=coord.match(/^(\d{1,3})\|(\d{1,3})$/); if(!m) return UI.ErrorMessage('Ziel kaputt');
    const tx=+m[1], ty=+m[2];
    const vills=load('vills',[]); if(!vills.length) return UI.ErrorMessage('Keine Dörfer geladen');
    const units=availableUnits();
    for(const v of vills){
      for(const u of units){
        const cell=$(`ax_time_${v.id}_${u}`); if(cell) cell.textContent='…';
  ...
  $('#ax_go').onclick = ()=>{
    ...
    // Erstes Dorf, in dem du was eingetragen hast, fliegt los. Du willst Kontrolle, du kriegst Kontrolle.
    for(const v of vills){
      const set=load('amounts',{})[v.id]||{};
      const payload={}; for(const u of units){ const n=+set[u]||0; if(n>0) payload[u]=n; }
  ...
  (function init(){
    ...
    $('#ax_target').value = load('target','');
    $('#ax_time').value   = load('time', fmtHMSms(new Date()));
